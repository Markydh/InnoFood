      ...
      <div class="son-right">
        <el-row type="flex" justify="center" align="middle">
          <el-form  ref="loginForm" :model="delivery" :rules="rules" status-icon label-width="80px">
            ...
            <router-link to="/deliveryRegister" style="color:rgb(143,121,198);text-decoration: none;font-size: small;margin-right: 120px;">Sign Up</router-link>
...
export default {
  ...
  methods: {
    doLogin() {
      if (!this.delivery.username) {
        this.$message.error("请输入用户名！");
        return;
      } else if (!this.delivery.password) {
        this.$message.error("请输入密码！");
        return;
      } else {
        //校验用户名和密码是否正确;
        // this.$router.push({ path: "/personal" });
        this.request.post("/delivery/login", this.delivery).then(res => {
